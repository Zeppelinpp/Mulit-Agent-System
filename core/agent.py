...
class PlanningAgent(BaseAgent):
    ...
    async def plan(self, query: str) -> TaskList:
        ...
        status_manager.update_agent_status(
            "Planner", "working", "Task Planning", "analyzing query and creating tasks"
        )
    ...
    async def report(self, task_list: TaskList, query: str):
        ...
        if success_tasks:
            ...
            for task in success_tasks:
                ...
                success_table.add_row(
                    task.task_name, task.goal, result_display, task.agent_name
                )
        ...
        if agent_stats:
            ...
            agent_table.add_column(
                "Total Tasks", style="white", width=12, justify="center"
            )
            ...
            agent_table.add_column(
                "Success Rate", style="yellow", width=15, justify="center"
            )
        ...
        if all_tasks:
            ...
            # Temporarily stop the agent status display to prevent conflicts
            status_manager.stop_live_display()

            # Stream tokens and render markdown simultaneously
            summary_text = ""
                    ...
                    if chunk.choices[0].delta.content:
                        ...
                        if summary_text.strip():
                            ...
                            live.update(
                                Panel(
                                    markdown_summary,
                                    title="[bold magenta]📝 Executive Summary[/bold magenta]",
...
class AgentStatus:
    ...
    def start_live_display(self):
        ...
        self.live = Live(
            self._generate_display(), console=self.console, refresh_per_second=4
        )
        self.live.start()
    ...
    def update_agent_status(
        self, agent_name: str, status: str, task_name: str = "", details: str = ""
    ):
    ...
    def _generate_display(self):
        ...
        return Panel(
            table,
            title="[bold blue]Agent Worker Status[/bold blue]",
            border_style="blue",
        )
...
class AgentWorker(BaseAgent):
    ...
    async def _execute_tool(self, tool_name: str, tool_args: Dict[str, Any]):
        status_manager.update_agent_status(
            self.name, "working", details=f"executing {tool_name}"
        )
    ...
    async def _execute_task(self, task: Task):
        ...
        status_manager.update_agent_status(
            self.name, "working", task.task_name, "starting task"
        )
        ...
        while True:
            status_manager.update_agent_status(
                self.name, "working", task.task_name, "thinking..."
            )
            ...
            if not tool_calls:
                ...
                task.status = "success"
                status_manager.update_agent_status(
                    self.name, "completed", task.task_name, "task finished"
                )
                return task
    ...
    async def run(self, result_queue: asyncio.Queue):
        ...
        while True:
            try:
                ...
                status_manager.update_agent_status(
                    self.name, "completed", task.task_name, "submitting result"
                )
                ...
                status_manager.update_agent_status(
                    self.name, "idle", "", "waiting for tasks"
                )
...
class AgentRunner:
    ...
    async def handle_result_loop(self):
        while True:
            ...
            try:
                status_manager.update_agent_status(
                    "Planner", "working", task.task_name, "reviewing task result"
                )
                ...
                if self.result_queue.empty():
                    status_manager.update_agent_status(
                        "Planner", "idle", "", "waiting for results"
                    )
